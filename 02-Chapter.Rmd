...
In this chapter, you will learn how to "read" a graphical representation of a gene as illustrated in the UCSC genome browser. The various named elements of a gene (exons, introns, 5′ UTR, coding sequence, etc.) are defined by their roles in gene expression—including transcription, RNA processing, and translation. For example, one continuous strand of genomic DNA is used as a template to produce a single-stranded pre-mRNA transcript. The beginning of a gene schematic in the genome browser is defined by where transcription begins. The end of a gene schematic in the browser is defined by where transcription ends. Before the pre-mRNA transcript is exported out into the cytoplasm, it is processed including the removal of internal segments called **introns**. These are represented in the genome browser as a thin line. The RNA segments that remain are called **exons** (or **ex**pressed regi**ons**). In the cytoplasm, mRNA becomes translated. Translation begins at a start codon and ends at a stop codon. The 5' UTR and 3' UTR are defined as the transcribed region of a gene that lie upstream (5' UTR) or downstream (3' UTR) of the translated region. These elements and more are uniquely illustrated in the genome browser such that they can easily be located. 
...
Importantly, ribosomes don't start translation at the 5' *end* of the mRNA but from a **"start codon"** located *internally*. Moreover, ribosomes translate only until they encounter a stop codon. Thus, codons *only occupy the central region of mRNA*. The 5' and 3' ends of the mRNA not "read" by the ribosome are called **"noncoding regions"** or **"untranslated regions" (UTRs)**. The noncoding region at the 5' end is called the **5’ untranslated region (5’ UTR)** while the noncoding region at the 3' end is called the **3’ untranslated region (3' UTR)**. 

Figure \@ref(fig:topstrand) shows how noncoding (blue box) and coding (green box) regions of an exon (red box) are illustrated in the UCSC Genome Browser. Notice how the noncoding portion is not as thick as the coding portion. At this zoom level, the coding portion also displays the amino acid sequence that this exon codes for: M-A-A-A-S-S-S-D-S-D-A-C-G-A-E-S (notice each amino acid is placed directly below a codon of three nucleotides). Again, notice how the amino acid sequence does not start at the *beginning* of exon 1 (red box). The thinner portion of exon 1 is the 5’ untranslated region (5’ UTR). Importantly, **exon 1 includes both the noncoding and coding portions**. To confirm that you are looking in the right place, the BBS1 5' UTR (the beginning of exon 1) begins with the sequence G-G-T-T. The coding portion of exon 1 begins with the sequence A-T-G-G. Exon 1 ends with the sequence, AGAG. The intron begins with the sequence G-T-G-A.
...
## Coding Sequence Determines Protein Sequence
...
<br>
```{r zoom, out.width='100%', fig.align='center', fig.cap = "There are three isoforms of DPP3 (DPP3-DT is not an isoform of DPP3). Notice how DPP3, highlighted with a red box, begins and ends transcription at the same location but the RNA transcript that is produced is spliced in three distinct ways to produce three alternative splice forms. One obvious difference is highlighted with a red arrow. The 2nd and 3rd splice forms have an exon not included in the topmost splice form listed. Each DDP3 splice form rightly has a unique mRNA ID. From top to bottom, there is NM_001256670.2, NM_005700.5 and NM_130443.4"}
knitr::opts_chunk$set(echo = FALSE)
...
## Plus-strand and Minus-strand genes
Chromosomal DNA is double-stranded. One strand is arbitrarily designated as the plus-strand in the reference genome assembly. The other strand is the minus (−) strand. In a genome browser, the plus strand defines the reference chromosome’s coordinate orientation with its 5′ end designated as position 1. Coordinate numbers then increase thereafter from 5′ to 3′. By default, genome browsers display the plus strand. The minus strand is reverse-complementary to the plus strand. Coordinate numbers decrease from 5′ to 3'. Importantly, genes can be located on either strand.

We learned in a previous chapter that the BBS1 coding sequence can be read in the plus-strand of the genome. This is because BBS1 uses the minus (−) strand as its template during transcription, producing a pre-mRNA whose sequence matches the plus-strand^[Except that Uracil replaces Thymine]. BBS1 is therefore a plus-strand gene. For all plus-strand genes, the plus-strand has been described as the *sense strand*, *coding strand*, *informational strand* or *nontemplate strand* ^[I prefer "nontemplate" or "informational strand". "Coding strand" is misleading because it implies that the entire transcript is coding - not true. "Sense strand" is not very informative. I like "informational strand" because this is the DNA strand where you can read sequence motifs in the RNA - for example, the codons or RNA binding sites that might impact translation or RNA stability. You can't argue with the term "nontemplate strand". It's clearly accurate], whereas, the minus-strand is the template strand.

There are a two main ways to determine if a gene is a plus-strand gene or a minus-strand gene. First, click anywhere in the gene prediction track to open the "item details" page (yellow pop up window). Scroll down to find the word, "Strand:" (Jump ahead to Figure \@ref(fig:geneinfo) to see what the page looks like). Alternatively, look at the gene schematic itself. Figure \@ref(fig:orientationwedges) displays the first half of BBS1 and a neighboring gene, ZDHHC24. You will find “orientation wedges” spaced regularly along the introns at any zoom level. They either point to the right (red box, Figure \@ref(fig:orientationwedges)) or to the left (green box, Figure \@ref(fig:orientationwedges)). These “orientation wedges” indicate the orientation of the nontemplate strand and the RNA that the gene produces. They also point in the direction that RNA polymerase travels during transcription. The “orientation wedges” for BBS1 point to the right indicating that BBS1 is a plus-strand gene while the orientation wedges for ZDHHC24 point to the left indicating that it is a minus-strand gene. If you zoom in enough, you will see orientation wedges everywhere (along exons and introns). 

<br>
```{r orientationwedges, out.width='60%', fig.align='center', fig.cap ="The orientation wedges are visible within introns at any zoom level. When they point to the right (red box), the gene is a top strand gene (plus strand). When they point to the left (green box), the gene is a minus-strand gene (minus strand)."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/orientationwedges.png')
```
<br>
To summarize, when orientation wedges point to the right, ">>>>",
<br>
1. This is a plus-strand gene where the beginning of the gene is on the left. 
2. The plus-strand of DNA is the nontemplate strand (the informational strand). 
3. The minus-strand of DNA is the template strand. 
4. The RNA sequence is identical to the plus strand (except Uracil replaces Thymine). 

<br>
When orientation wedges point to the left, "<<<<", the reverse is true.
<br>
1. This is a minus-strand gene where the beginning of the gene is on the right. 
2. The minus-strand of DNA is the nontemplate strand (the informational strand). 
3. The plus-strand of DNA is the template strand. 
4. The RNA sequence is identical to the minus strand (except Uracil replaces Thymine). 
<br>
...
<br>
Now let’s look at a minus-strand gene (ZDHHC24) more closely. Click on my saved session for [BBS1](https://genome.ucsc.edu/s/megallegos/Biol%20310%20Module%201){target="_blank"} then zoom out 3X to search for the **beginning** of ZDHHC24 (it should be on the right). "Drag-and-Select" as many times necessary to zoom into the **first** half of the **coding portion** of exon one for ZDHHC24. Your “NCBI Refseq Gene Track should look something like Figure \@ref(fig:firstcodingexonZDHHC24). If not, you may have zoomed into the first half of exon one which is noncoding. Or maybe you are at the wrong end of the gene! 


<br>
```{r firstcodingexonZDHHC24, out.width='100%', fig.align='center', fig.cap ="Here you are looking at a small portion of a minus strand gene (ZDHHC24) in the gene prediction track and the plus strand of the DNA in the base position track. The DNA sequence you see is the template strand for ZDHHC24 NOT the RNA sequence. To view the minus strand of DNA, click on the arrow."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/firstcodingexonZDHHC24.png')
```
<br>
```{r firstcodingexonZDHHC24bottom, out.width='100%', fig.align='center', fig.cap ="Here you are looking at the same small portion of ZDHHC24 in the gene prediction track and the minus strand of DNA in the base position track. Now the sequence is the same as the RNA sequence (except uracil replaces thymine). You can confirm this by looking at the start codon. It reads A-T-G in the DNA as expected."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/firstcodingexonZDHHC24Bottom.png')
```
<br>

Look at the three nucleotides centered above the Methionine (colored in green on the right side of Figure \@ref(fig:firstcodingexonZDHHC24)). Notice that the nucleotides listed from left to right is NOT "A-T-G" as you might expect but instead C-A-T! This is because you are “reading” the plus-strand of the chromosome and ZDHHC24 is a **minus (-)** strand gene. **To view the minus-strand of DNA**, click on the far left arrow within the Base Position Track (See the red oval, Figure \@ref(fig:firstcodingexonZDHHC24)). Notice the orientation of the arrow flips (red oval, \@ref(fig:firstcodingexonZDHHC24bottom)) and the genome sequence in your “Base Position” Track becomes light gray. *Now* there *is* an A-T-G directly above the Methionine (M) but only if read from the *right to left*! This is because the minus-strand of DNA is oriented with the 5’ end on the right.  

To summarize, genes use only one DNA strand as template (either the plus or minus strand). Plus-strand genes, like BBS1, create an RNA transcript with a sequence that matches the plus-strand. Minus strand genes, like ZDHHC24, create an RNA transcript with a sequence that matches the minus-strand.
...
Imagine you isolate and sequence a piece of DNA pulled from the ocean. You want to see if it corresponds to a coding portion of a gene and if so, if it codes for a previously studied protein. Of course, you have no clue which strand is normally used as the template-strand for transcription nor where the start codon is located. What should you do? Make no assumptions as they are all equally likely. Here is a thought question: ***What is the maximum number of polypeptide sequences that can hypothetically be created by a single segment of double-stranded DNA?*** Your answer is revealed below. 
...
You should now see three reading frames displayed below the nucleotide sequence. Why three? This is the maximum number of unique polypeptides that can be theoretically produced from a given segment of the plus-strand of DNA. See what happens when you toggle to the minus-strand of DNA (click the arrow ---> on the far left of the DNA sequence). Now you can view the three reading frames that correspond to the minus-strand of DNA. Both views are shown one on top of the other in Figure \@ref(fig:basepositionfullreverse). Notice they are different. 
...
```
<br>
```{r basepositionfullreverse, out.width='100%', fig.align='center', fig.cap="Once you change the base position track to **Full**, the three possible reading frames for a *single* DNA strand will be displayed. When the arrow points to the right, the three reading frames will be of the plus-strand (top image). Notice the start codon in green matches the start codon in the third reading frame (**NOTE: This image comes from the 2013 genome assembly and will look different for you.**) In fact, nearly all of the amino acids in exon one match the amino acids in the third reading frame. We expect one of the three plus-strand reading frames to match *BBS1* since BBS1 is a plus-strand gene. Now , however, when the arrow points to the left (circled in red), the three reading frames displayed are for the minus-strand (bottom image). Now *none* of the amino acids in the three reading frames match the amino acids found in *BBS1*."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/basepositionfullreverse.png')
```
<br>

The three reading frames corresponding to the plus-strand are numbered +1 (top), +2 (middle) and +3 (bottom). The three reading frames corresponding to the minus-strand are numbered -1 (top), -2 (middle) and -3 (bottom). For convenience, the position of all ATG codons (possible start codons) are highlighted in green and all possible stop codons (*) are highlighted in red. 
