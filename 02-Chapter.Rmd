...
There are a two main ways to determine if a gene is a plus-strand gene or a minus-strand gene. First, click anywhere in the gene prediction track to open the "item details" page (yellow pop up window). Scroll down to find the word, "Strand:" (Jump ahead to Figure \@ref(fig:geneinfo) to see what the page looks like). Alternatively, look at the gene schematic itself. Figure \@ref(fig:orientationwedges) displays the first half of BBS1 and a neighboring gene, ZDHHC24. You will find “orientation wedges” spaced regularly along the schematic. They either point to the right (red box, Figure \@ref(fig:orientationwedges)) or to the left (green box, Figure \@ref(fig:orientationwedges)). These “orientation wedges” point in the direction that RNA polymerase travels during transcription. Given that RNA polymerase travels from 5' to 3' relative to the nontemplate strand they indicate the orientation of the nontemplate strand. The “orientation wedges” for BBS1 point to the right indicating that BBS1 is a plus-strand gene while the orientation wedges for ZDHHC24 point to the left indicating that it is a minus-strand gene. 

<br>
```{r orientationwedges, out.width='60%', fig.align='center', fig.cap ="The orientation wedges are visible within introns. When they point to the right (red box), the gene is a top strand gene (plus strand). When they point to the left (green box), the gene is a minus-strand gene (minus strand)."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/orientationwedges.png')
```
<br>
To summarized, when orientation wedges point to the right, ">>>>",
<br>
1. This is a plus-strand gene.
2. The beginning of the gene is on the left, 5' end of the plus-strand. 
2. The plus-strand is the nontemplate strand (informational strand)
3. The minus-strand is the template strand. 

<br>
When orientation wedges point to the left, "<<<<", the reverse is true.
<br>
1. This is a minus-strand gene.
2. The beginning of the gene is on the right, 5' end of the minus-strand. 
2. The plus-strand is the template strand. 
3. The minus-strand is the nontemplate strand (informational strand).
<br>
...
<br>
```{r firstcodingexonZDHHC24, out.width='100%', fig.align='center', fig.cap ="This is the first half of the first **coding** exon of ZDHHC24. The noncoding portion of exon 1 extends to the right this time because *ZDHHC24* is a minus-strand gene. The red oval highlights the strand you are currently viewing. Since the arrow points to the right, the top strand (5' on the left) is displayed. You can toggle this arrow with your mouse."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/firstcodingexonZDHHC24.png')
```
<br>
```{r firstcodingexonZDHHC24bottom, out.width='100%', fig.align='center', fig.cap ="Again, this is the first half of the first **coding** exon of ZDHHC24. The red oval highlights the strand you are currently viewing. Since the arrow points to the left, the minus-strand (5' on the right) is displayed. Notice that the DNA sequence is gray. Compare to the top image. This is a visual reminder of which strand you are viewing."}
knitr::opts_chunk$set(echo = FALSE)
knitr::include_graphics('static/images/firstcodingexonZDHHC24Bottom.png')
```
<br>

Look at the three nucleotides centered above the Methionine (colored in green on the right side of Figure \@ref(fig:firstcodingexonZDHHC24)). Notice that the nucleotides listed from left to right is NOT "A-T-G" as you might expect but instead C-A-T! This is because you are “reading” the plus-strand of the chromosome and ZDHHC24 is a **minus (-)** strand gene. **To view the minus-strand of DNA**, click on the far left arrow within the Base Position Track (See the red oval, Figure \@ref(fig:firstcodingexonZDHHC24)). Notice the orientation of the arrow will flip and the genome sequence in your “Base Position” Track will become light gray (red oval, \@ref(fig:firstcodingexonZDHHC24bottom)). *Now* there is an A-T-G directly above the Methionine (M) but only if you read it from *right to left*! This is because the minus-strand of DNA is oriented with the 5’ end on the right.  

To summarize, genes use only one DNA strand as template (either the plus or minus strand). Plus-strand genes, like BBS1, use the minus-strand as template to create an RNA transcript with a sequence that matches the plus-strand. Other genes, like ZDHHC24 (a minus-strand gene), use the plus-strand as template to create an RNA transcript with a sequence that matches the minus-strand.  
